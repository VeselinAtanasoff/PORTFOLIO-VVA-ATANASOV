...
void Ui::OnDataRecv(const uint8_t *mac_addr, const uint8_t *current_data)
{
    memcpy(&receivedMessage, current_data, sizeof(receivedMessage));

    Device device;

    memcpy(device.macAddress.MA, mac_addr, sizeof(device.macAddress.MA));

    device.message.messageType = receivedMessage.messageType;
    device.message.data = receivedMessage.data;

    display.showErrorCode(device);
}
